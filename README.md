...
```bash
ConsumerFalabellaNodeJSTest$ sam build
```
...
```bash
ConsumerFalabellaNodeJSTest$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ConsumerFalabellaNodeJSTest$ sam local start-api
ConsumerFalabellaNodeJSTest$ curl http://localhost:3000/
```
...
```bash
ConsumerFalabellaNodeJSTest$ sam logs -n HelloWorldFunction --stack-name ConsumerFalabellaNodeJSTest --tail
```
...
```bash
ConsumerFalabellaNodeJSTest$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name ConsumerFalabellaNodeJSTest
```
